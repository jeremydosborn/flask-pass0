...
# NOTE: PASS0_AUTO_APPROVE_DEVICES removed – devices always require explicit approval
...
@app.route('/login')
def login_page():
    """Login UI: renders auth.html, used via Pass0's /auth/login redirect stub."""
    user = get_current_user()
    if user:
        return redirect('/')
...
def log_auth_events(response):
    ...
    if request.endpoint == 'pass0.verify':
        ...
        if response.status_code == 302 and 'user_id' in session:
            ...
            if user:
                log_event(
                    'step_6_token_verified',
                    'Token verified in database',
                    user_id=user['id'],
                    email=user['email']
                )
                log_event(
                    'step_7_session_created',
                ...
                if hasattr(pass0, 'device_binding') and pass0.device_binding:
                    ...
                    if is_trusted:
                        log_event(
                            'step_8_device_recognized',
                            'Existing trusted device detected',
                            user_id=user['id']
                        )
                    else:
                        log_event(
                            'step_8_device_new',
                            'New device - adding to trusted devices',
                            user_id=user['id'],
        ...
        if response.status_code == 200:
            ...
            if user:
                try:
                    ...
                    log_event(
                        'devices_listed',
                        f'Retrieved {device_count} trusted devices',
                        user_id=user['id']
                    )
                except:
                    pass
